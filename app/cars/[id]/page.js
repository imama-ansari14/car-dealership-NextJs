...
// Update UI component imports with explicit extensions to prevent resolution failure
import { cars } from "@/data/cars";
import CarCard from "@/components/ui/CarCard.jsx";
import Badge from "@/components/ui/Badge.jsx";
import { SectionLabel } from "@/components/ui/Button.jsx";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d0d" }}>
      <div style={{ paddingTop: "7.5rem", paddingBottom: "1.5rem", maxWidth: 1400, margin: "0 auto", padding: "7.5rem 3rem 1.5rem" }}>
        <Link
          href="/inventory"
          style={{ display: "inline-flex", alignItems: "center", gap: ".5rem", color: "rgba(255,255,255,.35)", textDecoration: "none", fontFamily: "'Barlow Condensed',sans-serif", fontSize: ".7rem", letterSpacing: ".2em", textTransform: "uppercase", transition: "color .3s" }}
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 3rem 6rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", marginBottom: "5rem", alignItems: "start" }}>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.1 }} style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <div>
              <p className="font-condensed" style={{ color: "rgba(255,255,255,.35)", fontSize: ".72rem", letterSpacing: ".3em", textTransform: "uppercase", marginBottom: ".5rem" }}>
                {car.brand} · {car.year}
              </p>
              <h1 className="font-display" style={{ fontSize: "clamp(2rem,4vw,3rem)", color: "#fff", margin: "0 0 1.25rem", lineHeight: 1.1 }}>
                {car.model}
              </h1>
            ...
            <div style={{ display: "flex", gap: "1.5rem", padding: "1.25rem 0", borderTop: "1px solid rgba(255,255,255,.06)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
              {[
                { val: car.horsepower, unit: "HP" },
                { val: car.topSpeed.replace(" mph", ""), unit: "Top Speed" },
                { val: car.acceleration.split("s")[0] + "s", unit: "0–60 mph" },
              ].map((s, i) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".6rem" }}>
              {[
                ...
                { l: "Mileage", v: car.mileage === 0 ? "Brand New" : car.mileage.toLocaleString() + " mi" },
                { l: "Transmission", v: car.transmission.split(" ").slice(0, 2).join(" ") },
              ].map((item) => (
            ...
            <div className="glass-gold" style={{ padding: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: ".75rem", flexWrap: "wrap" }}>
                <Link href="/booking" style={{ flex: 1, textDecoration: "none", minWidth: 130 }}>
                  <motion.span whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="btn-gold" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: ".85rem 1rem", fontSize: ".72rem", letterSpacing: ".1em" }}>
                ...
                <Link href="/contact" style={{ flex: 1, textDecoration: "none", minWidth: 130 }}>
                  <motion.span whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="btn-outline-gold" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: ".85rem 1rem", fontSize: ".72rem", letterSpacing: ".1em" }}>
                    Enquire Now
                  </motion.span>
                </Link>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }} style={{ marginBottom: "5rem" }}>
        ...
        {related.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
              ...
              <Link
                href="/inventory"
                style={{ display: "flex", alignItems: "center", gap: ".5rem", color: "rgba(255,255,255,.35)", textDecoration: "none", fontFamily: "'Barlow Condensed',sans-serif", fontSize: ".7rem", letterSpacing: ".2em", textTransform: "uppercase", transition: "color .3s" }}
                onMouseEnter={(e) => (e.currentTarget.style.color = "#c9a84c")}
                onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(255,255,255,.35)")}
              >
